...
######################################################
########## True Win Probability/Win Ratio for revision ############
######################################################
...
for (count_temp in 1:sim_num){
  ...
  print(count_temp)
  # covariates
  x = rmvnorm(n_count,mean=mean_set,sigma=covariance_set)
  x1 <- x[,1]
  x2 <- x[,2]
  x3 <- x[,3]
  x4 <- x[,4]
  ...
  logodds1_trt_assigned <- b01 + as.matrix(ctrl_cov) %*% bi_trt + trt_eff1
  logodds2_trt_assigned <- b02 + as.matrix(ctrl_cov) %*% bi_trt + trt_eff1
  
  # trt but assigned in ctrl group
  logodds1_ctrl_assigned <- b01 + as.matrix(trt_cov) %*% bi_ctrl
  logodds2_ctrl_assigned <- b02 + as.matrix(trt_cov) %*% bi_ctrl
  ## Probability trt
...
run_time
#Time difference of 1.13116 hours   p=0.5
#Time difference of 1.142874 hours  p=0.7
...
#   0.3736102       0.2429874   1.546113  0.1306228


#unbalanced design (pi=0.7) with sample size=500
#WP_trt_trueVal WP_ctrl_trueVal WR_trueVal    WD_trueVal
#0.3726256       0.2437491   1.537628  0.1288766
