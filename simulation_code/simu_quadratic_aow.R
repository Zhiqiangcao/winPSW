...
while(k <= N){
  ...
  if(ltrt !=3 | lctrl != 3){
    ...
    aow_res = aow_estimation(data=mydata,outcomevar="outcome",treatment="treatment",
                             ...
                             covariate_reg=c("x1","x2","x3","x4","x5","x6","x12","x13","x14","x15","x16",
                                             "x23","x24","x25","x26","x34","x35","x36","x45","x46","x56"))
    est1[k,] = aow_res$EST
    se1[k,] = aow_res$SE
    low1 = aow_res$conf_low
    high1 = aow_res$conf_high
...
res1
final_res = res1
write.csv(final_res,file="C:/Users/82655/Dropbox/research/covariate adjustment win ratio/simu_res_int/aow_b_200.csv")  
