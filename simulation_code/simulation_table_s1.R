###simulation of unadjusted for table S1
rm(list=ls())
library(MASS)
library(nnet)
library(ggplot2)
...
gen_data_quad = function(n,outcomes_3lvl,bi_trt,bi_ctrl,b01,b02,sds,bern_param,pe){
  # covariates
...
#input source code
source("C:/Users/82655/Dropbox/research/covariate adjustment win ratio/simulation_code/function_setup_new1.R")

#main simulation
for(k in 1:N){
  set.seed(123456+k)
  if(quad==1){
    mydata = gen_data_quad(n,outcomes_3lvl,bi_trt,bi_ctrl,b01,b02,sds,bern_param,pe)
  }else{
    mydata = gen_data_int(n,outcomes_3lvl,bi_trt,bi_ctrl,b01,b02,sds,bern_param,pe)
  }
  #variance estimation by our method
  ...
  ##variance estimation by Bebu and Lachin (2016).
  unadj_res1 = unadj_estimation_BL(data=mydata,outcomevar="outcome",treatment="treatment")
  est2[k,] = unadj_res1$EST
  se2[k,] = unadj_res1$SE
  low2 = unadj_res1$conf_low
  high2 = unadj_res1$conf_high
  cova2[k,] = as.numeric(low2<=true_val&true_val<=high2)
  
  ###variance estimation by corrected method (Ozenne et al., 2021)
  unadj_res2 = unadj_estimation_Ozenne(data=mydata,outcomevar="outcome",treatment="treatment")
  est3[k,] = unadj_res2$EST
  se3[k,] = unadj_res2$SE
  low3 = unadj_res2$conf_low
  high3 = unadj_res2$conf_high
...
#for unadjusted
est1 = est1[est1[,2]!=0,]
wp_est = apply(est1,2,mean,na.rm=TRUE)
wp_sd = apply(est1,2,sd,na.rm=TRUE)
wp_se = apply(se1,2,mean,na.rm=TRUE)
wp_cr = apply(cova1,2,mean,na.rm=TRUE)
res1 = data.frame(true_val,wp_est,wp_sd,wp_se,wp_cr)
row.names(res1) = c("WP","LP","WR","WD")
res1
#for BL method
est2 = est2[est2[,2]!=0,]
wp_est = apply(est2,2,mean,na.rm=TRUE)
wp_sd = apply(est2,2,sd,na.rm=TRUE)
wp_se = apply(se2,2,mean,na.rm=TRUE)
wp_cr = apply(cova2,2,mean,na.rm=TRUE)
...
#for Ozenne method
est3 = est3[est3[,2]!=0,]
wp_est = apply(est3,2,mean,na.rm=TRUE)
wp_sd = apply(est3,2,sd,na.rm=TRUE)
wp_se = apply(se3,2,mean,na.rm=TRUE)
wp_cr = apply(cova3,2,mean,na.rm=TRUE)
res3 = data.frame(true_val,wp_est,wp_sd,wp_se,wp_cr)
row.names(res3) = c("WP","LP","WR","WD")
res3
final_res = rbind(res1,res2,res3)
final_res
write.csv(round(final_res,3),file=paste0("quad=", quad, "_choose_", choose, "_n_", n, ".csv"))
